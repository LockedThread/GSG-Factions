...
public class CmdUnclaim extends FCommand {
    ...
    private boolean unClaim(FLocation target) {
        ...
        Access access = targetFaction.getAccess(fme, PermissableAction.TERRITORY);
        if (access == Access.DENY || (access == Access.UNDEFINED && !assertMinRole(Role.MODERATOR))) {
            fme.msg(TL.GENERIC_NOPERMISSION, "unclaim");
            return false;
        }
        ...
        for (Entity entity : target.getChunk().getEntities()) {
            if (entity instanceof Player) {
                ...
                FPlayer fplayer = FPlayers.getInstance().getByPlayer(plr);
