...
public class FPromoteCommand extends FCommand {
    ...
    public void perform() {
        ...
        // Don't allow people to promote others to their same or higher rank.
        if (promotion.value >= fme.getRole().value) {
            fme.msg(TL.COMMAND_PROMOTE_NOT_ALLOWED);
            return;
        }

        // Don't allow people to demote people who are the same or higher rank.
        if (fme.getRole().value <= current.value) {
            fme.msg(TL.COMMAND_DEMOTE_NOT_ALLOWED);
            return;
        }
