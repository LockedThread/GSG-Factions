...
public class CmdKick extends FCommand {
    ...
    public void perform() {
        ...
        if (toKick == null) {
            ...
            for (FPlayer player : myFaction.getAltPlayers()) {
                String s = player.getName();
                msg.then(s + " ").color(ChatColor.WHITE).tooltip(TL.COMMAND_KICK_CLICKTOKICK.toString() + s).command("/" + Conf.baseCommandAliases.get(0) + " kick " + s);
            }
        ...
        if (!toKickFaction.isNormal() && toKick.hasAltFaction()) {
            toKickFaction = toKick.getAltFaction();
        }
        ...
        if (!Permission.KICK_ANY.has(sender)) {
            ...
            if (!toKick.hasAltFaction() && toKick.getRole().value >= fme.getRole().value) {
        ...
        if (toKick.hasFaction() && toKick.getRole() == Role.ADMIN) {
        ...
        if (toKick.hasFaction()) {
            ...
            toKickFaction.deinviteAlt(toKick);
            toKick.resetAltFactionData();
        }
    }
