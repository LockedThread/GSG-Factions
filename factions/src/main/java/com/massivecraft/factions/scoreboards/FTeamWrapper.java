...
public class FTeamWrapper {
    ...
    private static final Map<Faction, com.massivecraft.factions.scoreboards.FTeamWrapper> wrappers = new HashMap<>();
    private static final List<com.massivecraft.factions.scoreboards.FScoreboard> tracking = new ArrayList<>();
    private static final Set<Faction> updating = new HashSet<>();
    private static int factionTeamPtr;
    private final Map<com.massivecraft.factions.scoreboards.FScoreboard, Team> teams = new HashMap<>();
    ...
    private FTeamWrapper(Faction faction) {
        ...
        for (com.massivecraft.factions.scoreboards.FScoreboard fboard : tracking) {
            add(fboard);
        }
    }

    public static void applyUpdatesLater(final Faction faction) {
        if (!com.massivecraft.factions.scoreboards.FScoreboard.isSupportedByServer()) {
            return;
        }
    ...
    public static void applyUpdates(Faction faction) {
        ...
        com.massivecraft.factions.scoreboards.FTeamWrapper wrapper = wrappers.get(faction);
        ...
        if (wrapper == null) {
            wrapper = new com.massivecraft.factions.scoreboards.FTeamWrapper(faction);
    ...
    protected static void track(com.massivecraft.factions.scoreboards.FScoreboard fboard) {
        if (!com.massivecraft.factions.scoreboards.FScoreboard.isSupportedByServer()) {
            return;
        }
        tracking.add(fboard);
        for (com.massivecraft.factions.scoreboards.FTeamWrapper wrapper : wrappers.values()) {
            wrapper.add(fboard);
        }
    }

    protected static void untrack(com.massivecraft.factions.scoreboards.FScoreboard fboard) {
        if (!com.massivecraft.factions.scoreboards.FScoreboard.isSupportedByServer()) {
            return;
        }
    ...
    private void add(com.massivecraft.factions.scoreboards.FScoreboard fboard) {
    ...
    private void remove(com.massivecraft.factions.scoreboards.FScoreboard fboard) {
        teams.remove(fboard).unregister();
    }

    private void updatePrefixes() {
        if (P.p.getConfig().getBoolean("scoreboard.default-prefixes", false)) {
            for (com.massivecraft.factions.scoreboards.FScoreboard fboard : teams.keySet()) {
                updatePrefix(fboard);
            }
        }
    }
