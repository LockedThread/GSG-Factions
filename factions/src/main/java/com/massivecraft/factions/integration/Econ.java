...
public class Econ {
    ...
    public static void sendBalanceInfo(FPlayer to, EconomyParticipator about) {
        ...
        to.msg("<a>%s's<i> balance is <h>%s<i>.", about.describeTo(to, true), moneyString(econ.getBalance(about.getAccountId())));
    }
    ...
    public static void sendBalanceInfo(CommandSender to, Faction about) {
        ...
        to.sendMessage(String.format("%s's balance is %s.", about.getTag(), moneyString(econ.getBalance(about.getAccountId()))));
    }
    ...
    public static boolean transferMoney(EconomyParticipator invoker, EconomyParticipator from, EconomyParticipator to, double amount, boolean notify) {
        if (!shouldBeUsed()) {
            invoker.msg(TL.ECON_OFF, ", the econ is disabled.");
            return false;
        }
        ...
        if (isUUID(from.getAccountId())) {
            OfflinePlayer offline = Bukkit.getOfflinePlayer(UUID.fromString(from.getAccountId()));
            if (offline.getName() == null) {
                return false;
            }
            fromAcc = offline.getName();
        } else {
            fromAcc = from.getAccountId();
        }

        if (isUUID(to.getAccountId())) {
            OfflinePlayer offline = Bukkit.getOfflinePlayer(UUID.fromString(to.getAccountId()));
            if (offline.getName() == null) {
                return false;
            }
            toAcc = offline.getName();
        } else {
            toAcc = to.getAccountId();
        }
    ...
    public static boolean modifyMoney(EconomyParticipator ep, double delta, String toDoThis, String forDoingThis) {
        ...
        if (isUUID(ep.getAccountId())) {
            OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(UUID.fromString(ep.getAccountId()));
            if (offlinePlayer.getName() == null) {
                return false;
            }
            acc = offlinePlayer.getName();
        } else {
            acc = ep.getAccountId();
        }
