        ...
        if (this.enabled = root.getBoolean("enabled")) {
            ...
            this.menuItem = MenuItem.of(ItemStackBuilder.of(root.getConfigurationSection("gui-item")).build()).setInventoryClickEventConsumer(event -> {
                ...
                if (integer == null) {
                    ...
                    if (costPair.getKey().purchase(player, costPair.getValue())) {
                        ...
                        player.sendMessage(TL.FACTION_UPGRADES_CANT_AFFORD.toString().replace("{level}", String.valueOf(1)).replace("{upgrade}", getPrettyName()).replace("{cost}", String.valueOf(costPair.getValue())));
                    }
