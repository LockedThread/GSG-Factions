...
public abstract class MemoryFPlayer implements FPlayer {
    ...
    public MemoryFPlayer(com.massivecraft.factions.zcore.persist.MemoryFPlayer other) {
    ...
    public int getKills() {
        return isOnline() ? getPlayer().getStatistic(Statistic.PLAYER_KILLS) : factionStatisticMap.getOrDefault(FactionStatistic.KILLS, 0);
    }

    public int getDeaths() {
        return isOnline() ? getPlayer().getStatistic(Statistic.DEATHS) : factionStatisticMap.getOrDefault(FactionStatistic.DEATHS, 0);
    }

    public int getTimePlayed() {
        return isOnline() ? getPlayer().getStatistic(Statistic.PLAY_ONE_TICK) / 20 : factionStatisticMap.getOrDefault(FactionStatistic.TIME_PLAYED, 0);
    }
    ...
    public boolean hasAltFaction() {
        return this.altFactionId != null && !altFactionId.isEmpty();
    }

    public Faction getAltFaction() {
        if (this.altFactionId != null && !altFactionId.isEmpty()) {
            return Factions.getInstance().getFactionById(this.altFactionId);
        }
        return null;
    }

    public void setAltFaction(Faction faction) {
        this.altFactionId = faction.getId();
    }

    public String getAltFactionId() {
        return altFactionId == null ? "" : altFactionId;
    }
    ...
    public void resetAltFactionData() {
        if (altFactionId != null && Factions.getInstance().isValidFactionId(this.getAltFactionId())) {
            Faction currentFaction = this.getAltFaction();
    ...
    public String getNameAndTitle(com.massivecraft.factions.zcore.persist.MemoryFPlayer fplayer) {
    ...
    public String getChatTag() {
        return this.hasFaction() ? String.format(Conf.chatTagFormat, this.getRole().getPrefix() + this.getTag()) : "";
    }
    ...
    public boolean isInOwnTerritory() {
        ...
        return hasAltFaction() ? faction == this.getAltFaction() : faction == this.getFaction();
    }

    public boolean isInOthersTerritory() {
        Faction factionHere = Board.getInstance().getFactionAt(new FLocation(this));
        return factionHere != null && factionHere.isNormal() && factionHere != (hasAltFaction() ? getAltFaction() : getFaction());
    }
...
//        }
    ...
    public boolean canUseFactionVault() {
        if (getFaction().isWilderness() || getFaction().isWarZone() || getFaction().isSafeZone()) {
            return false;
        }

        if (getPlayer().hasPermission("factions.vault.bypass")) {
            return true;
        }

        if (isAdminBypassing || getRole() == Role.ADMIN) {
            return true;
        }

        Access access = getFaction().getAccess(this, PermissableAction.CHEST);
