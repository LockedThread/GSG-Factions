...
public enum TagReplacer {
    ...
    ALTS_ONLINE_LIST(TagType.FANCY, "{alts-online-list}"),
    ALTS_OFFLINE_LIST(TagType.FANCY, "{alts-offline-list}"),
    ...
    ALTS_ONLINE_COUNT(TagType.FACTION, "{alts-online}"),
    ALTS_OFFLINE_COUNT(TagType.FACTION, "{alts-offline}"),
    FACTION_SIZE(TagType.FACTION, "{members}"),
    FACTION_ALTS_SIZE(TagType.FACTION, "{alts}"),
    ...
    protected static List<com.massivecraft.factions.zcore.util.TagReplacer> getByType(TagType type) {
        List<com.massivecraft.factions.zcore.util.TagReplacer> tagReplacers = new ArrayList<>();
        for (com.massivecraft.factions.zcore.util.TagReplacer tagReplacer : values()) {
    ...
    protected String getValue() {
        switch (this) {
            ...
            case FACTIONLESS:
                return String.valueOf(FPlayers.getInstance().getOnlinePlayers().stream()
                        .filter(fp -> Objects.equals(fp.getFactionId(), Factions.getInstance().getWilderness().getId())).count());
            case MAX_ALLIES:
    ...
    protected String getValue(Faction fac, FPlayer fp) {
        ...
        if (fp != null) {
            switch (this) {
                ...
                case PLAYER_GROUP:
                    return ""; // don't parse groups, unnecessary offlineplayer lookup for data we don't need
                case PLAYER_BALANCE:
        ...
        switch (this) {
            ...
            case ALTS_ONLINE_COUNT:
                return String.valueOf(fac.getAltPlayers().stream().filter(FPlayer::isOnline).count());
            case ALTS_OFFLINE_COUNT:
                return String.valueOf(fac.getAltPlayers().stream().filter(FPlayer::isOffline).count());
            case FACTION_SIZE:
                return String.valueOf(fac.getFPlayers().size());
            case FACTION_ALTS_SIZE:
                return String.valueOf(fac.getAltSize());
            case FACTION_KILLS:
